...
namespace Clean.Win.AppUI.UICommon
{
    ...
    public sealed class UICommon
    {
        ...
        {
            if (IsImplementedUIPermision(null, nameof(MenuEnums.Threads)))
            {
        ...
        {
            if (IsImplementedUIPermision(null, nameof(MenuEnums.Suppliers)))
            {
                var pnlMainContent = this.clearControls(mainForm);
                if (pnlMainContent != null)
                {
        ...
        {
            if (IsImplementedUIPermision(null, nameof(MenuEnums.UserManagements)))
            {
                var pnlMainContent = this.clearControls(mainForm);
                if (pnlMainContent != null)
                {
                    var uc = new ucUserManagement(mainForm._userCommandServices, mainForm._partQueryService,
                        mainForm._roleCommandServices);
                    uc.Dock = DockStyle.Fill;
                    pnlMainContent.Controls.Add(uc);
                }
            }
        ...
        {
            if (IsImplementedUIPermision(null, nameof(MenuEnums.UserGroups)))
            {
                var pnlMainContent = this.clearControls(mainForm);
                if (pnlMainContent != null)
                {
                    var uc = new ucUserGroups(mainForm._userGroupCommandServices, mainForm._userGroupQueryServicess,
                        mainForm._permissionCommandServices, mainForm._roleCommandServices);
                    uc.Dock = DockStyle.Fill;
                    pnlMainContent.Controls.Add(uc);
                }
            }
        ...
        public void DisplayingComputerConfigurationInformation(MainForm mainForm)
        {
            if (IsImplementedUIPermision(null, nameof(MenuEnums.UserGroups)))
            {
                ...
                if (pnlMainContent != null)
                {
                    var uc = new ucComputer(mainForm._computerCommandServices, mainForm._computerQueryServices,
                        mainForm._languageQueryService, mainForm._systemConfigurationCommandServices);
                    uc.Dock = DockStyle.Fill;
                    pnlMainContent.Controls.Add(uc);
                }
            }
        ...
        public void DisplayingBobbinsInformation(MainForm mainForm)
        {
            if (IsImplementedUIPermision(null, nameof(MenuEnums.Bobbins)))
            {
                var pnlMainContent = this.clearControls(mainForm);
                if (pnlMainContent != null)
                {
                    var uc = new ucBobbins(mainForm._bobbinCommandServices, mainForm._bobbinQueryServices);
                    uc.Dock = DockStyle.Fill;
                    pnlMainContent.Controls.Add(uc);
                }
            }
        ...
        {
            if (IsImplementedUIPermision(null, nameof(MenuEnums.Parts)))
            {
                var pnlMainContent = this.clearControls(mainForm);
                if (pnlMainContent != null)
                {
        ...
        public void DisplayingSewingMachineInformation(MainForm mainForm)
        {
            if (IsImplementedUIPermision(null, nameof(MenuEnums.Machines)))
            {
                var pnlMainContent = this.clearControls(mainForm);
                if (pnlMainContent != null)
                {
                    var uc = new ucMachine(mainForm._changeNeedleCommandServices, mainForm._clipSensorActivationCommandServices, mainForm._sewingMachineConfigurationCommandServices);
                    uc.Dock = DockStyle.Fill;
                    pnlMainContent.Controls.Add(uc);
                }
            }
        ...
        public void DisplayingArticleInformation(MainForm mainForm, MenuEnums mainMenu)
        {
            if (IsImplementedUIPermision(null, nameof(MenuEnums.Machines)))
            {
                if (mainForm != null)
                {
                    ...
                    if (pnlMainContent != null)
                    {
                        //before add new usercontrol into pnlMainContent, we need to clear existing sub-controls first
                        if (pnlMainContent.Controls.Count > 0)
                            pnlMainContent.Controls.Clear();

                        var drgHeaderNames = CleanWinFUtility.ConvertStringToArray(UIConstants.GRID_COLUMNS_ARTICLE_HEADER_Name, "||");
                        var drgHeaderTexts = CleanWinFUtility.ConvertStringToArray(UIConstants.GRID_COLUMNS_ARTICLE_HEADER_Text, "||");
                        var ucArticle = GetUserArticleControl(
                            mainForm._articleCommandService,
                            mainForm._articleQueryService,
                            mainForm._automatCommandServices,
                            mainForm._automatQueryServices,
                            mainForm._partCommandService,
                            mainForm._partQueryService,
                            mainForm._threadCommandServices,
                            mainForm._threadQueryServices,
                            nameof(mainMenu),
                            drgHeaderNames,
                            drgHeaderTexts);
                        ucArticle.Dock = DockStyle.Fill;
                        pnlMainContent.Controls.Add(ucArticle);
                    }
                }
            }
        }
